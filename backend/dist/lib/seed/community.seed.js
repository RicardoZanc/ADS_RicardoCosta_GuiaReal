...
export async function seedCommunity({ nodes, users, products, }) {
    const [rgOpinion, stratOpinion, monitorOpinion, floydNodeOpinion, ipsNodeOpinion, xpsOpinion] = await Promise.all([
        prisma.opinions.create({
            data: {
                user_id: users.admin.id,
                product_id: products.rg350dxz.id,
                title: "Excelente para Shred, mas a ponte exige paciência",
                content: "O braço é extremamente confortável e a captação responde bem ao ganho. No entanto, a ponte de microafinação Floyd Rose torna a troca de cordas demorada.",
                status: "PENDING",
                cached_upvotes: 5,
            },
        }),
        prisma.opinions.create({
            data: {
                ...
                content: "Som limpo cristalino e drive suave. A ponte fixa facilita manutenção e afinação estável em turnês.",
                status: "PROCESSED",
                cached_upvotes: 3,
            },
        }),
        prisma.opinions.create({
            data: {
                ...
                content: "O painel IPS entrega ângulos de visão excelentes para edição de fotos. O custo-benefício só compensa para uso profissional.",
                status: "PENDING",
                cached_upvotes: 2,
            },
        }),
        prisma.opinions.create({
            data: {
                ...
                content: "A Floyd Rose permite bends agressivos sem perder afinação, mas exige conhecimento para trocar cordas e ajustar contrapressão.",
                status: "PROCESSED",
                cached_upvotes: 8,
            },
        }),
        prisma.opinions.create({
            data: {
                ...
                content: "Painéis IPS mantêm consistência de cor entre diferentes ângulos, ideal para design e fotografia.",
                status: "PENDING",
                cached_upvotes: 1,
            },
        }),
        prisma.opinions.create({
            data: {
                ...
                content: "Desempenho excelente em compilação e renderização. O chassi de alumínio ajuda, mas ventoinhas ficam audíveis em cargas sustentadas.",
                status: "PENDING",
                cached_upvotes: 4,
            },
        }),
    ...
    const rgThread = await prisma.discussion_threads.create({
        data: {
            ...
            user_id: users.user.id,
            content: "Vale a pena lubrificar os carrinhos da ponte para não perder a afinação com bends agressivos.",
            status: "PENDING",
            cached_upvotes: 3,
        },
    });
    const rgReply = await prisma.discussion_threads.create({
        data: {
            ...
            user_id: users.mod.id,
            content: "Concordo. Uso óleo de máquina de costura nos pivôs — faz diferença na estabilidade.",
            status: "PENDING",
            cached_upvotes: 2,
        },
    });
    ...
    const floydThread = await prisma.discussion_threads.create({
        data: {
            ...
            user_id: users.user.id,
            content: "Ponte fixa mantém melhor afinação para iniciantes que ainda não dominam ajuste de contrapressão.",
            status: "PROCESSED",
            cached_upvotes: 6,
        },
    });
    const monitorThread = await prisma.discussion_threads.create({
        data: {
            ...
            user_id: users.user.id,
            content: "Para gaming, um painel 144Hz pode ser mais interessante que IPS puro nesta faixa de preço.",
            status: "PENDING",
            cached_upvotes: 0,
        },
    });
    const processedThread = await prisma.discussion_threads.create({
        data: {
            opinion_id: floydNodeOpinion.id,
            user_id: users.mod.id,
            content: "Com contrapressão bem calibrada, a Floyd Rose segura afinação mesmo com uso intenso de tremolo.",
            status: "PROCESSED",
            cached_upvotes: 7,
        },
    });
