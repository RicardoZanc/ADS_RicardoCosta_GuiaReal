...
const listPendingInteractions = async (query) => {
    ...
    const data = pendingThreads.map((thread) => ({
        thread_id: thread.id,
...
const createFact = async (input) => {
    ...
    if (uniqueThreadIds.length !== input.evidence_thread_ids.length) {
        throw new BadRequestError("evidence_thread_ids não pode conter valores duplicados");
    }
    ...
    if (nonPending.length > 0) {
        throw new BadRequestError("Todas as threads de evidência devem estar com status PENDING");
    }
...
const markInteractionProcessed = async (threadId) => {
...
const listByNode = async (nodeId) => {
    ...
    return {
        data: facts.map((fact) => ({
            ...
            evidence_thread_ids: fact.fact_evidence.map((evidence) => evidence.interaction_id),
        })),
    };
};
