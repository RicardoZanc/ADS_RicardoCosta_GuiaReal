...
async function resolveContext(productId, nodeId) {
    if (productId) {
...
function buildDiscussionPath(context, highlightType, highlightId) {
...
function mapThreadToItem(thread, parentId, depth, isEvidence) {
    return {
        id: thread.id,
        kind: "thread",
        parent_id: parentId,
...
async function resolveOpinionEvidence(opinionId) {
    ...
    const threadItems = [
        ...
        ...topLevelThreads.map((thread) => mapThreadToItem(thread, opinion.id, 1, false)),
    ];
    const rootOpinion = {
        id: opinion.id,
        title: opinion.title,
...
async function resolveThreadEvidence(threadId) {
    ...
    let depth = 1;
    let parentId = opinion.id;
    for (const ancestor of ancestorChain) {
        threadItems.push(mapThreadToItem(ancestor, parentId, depth, false));
        parentId = ancestor.id;
        depth += 1;
    }
    threadItems.push(mapThreadToItem(evidenceThread, evidenceThread.parent_interaction_id ?? opinion.id, depth, true));
    const siblingDepth = depth;
    const siblingParentId = evidenceThread.parent_interaction_id ?? opinion.id;
    for (const sibling of siblings) {
        threadItems.push(mapThreadToItem(sibling, siblingParentId, siblingDepth, false));
    }
    const rootOpinion = {
        id: opinion.id,
        title: opinion.title,
