...
import type { SeedNodes } from "./types";

export async function seedNodes(): Promise<SeedNodes> {
  const root = await prisma.nodes.create({
    data: { name: "ROOT_GLOBAL", type: node_type.ROOT },
  });

  const [tipoInstrumentos, tipoEletronicos, tipoEletrodomesticos] =
    await Promise.all([
      prisma.nodes.create({
        data: {
  ...
  const [catGuitarras, catBaixos, catMonitores, catNotebooks] =
    await Promise.all([
      prisma.nodes.create({
        data: {
          name: "Guitarras",
