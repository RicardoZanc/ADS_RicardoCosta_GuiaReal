...
export function assertValidAdminRequestMessage(message: string): string {
...
export async function loadUserForAdminRequest(
  userId: string
): Promise<UserForEligibility> {
...
export async function computeAdminRequestEligibility(
  user: UserForEligibility
): Promise<AdminRequestEligibility> {
...
export async function assertCanCreateAdminRequest(
  userId: string,
  message: string
): Promise<string> {
  ...
  if (!eligibility.can_request) {
    switch (eligibility.reason) {
      ...
      case "LOW_REPUTATION":
        throw new BadRequestError(
          `Reputação mínima de ${MIN_REPUTATION_FOR_ADMIN_REQUEST} pontos é necessária`
        );
      case "PENDING_REQUEST":
        throw new ConflictError("Você já possui uma solicitação pendente");
      case "COOLDOWN":
        throw new ConflictError(
          `Aguarde até ${eligibility.cooldown_ends_at} para solicitar novamente`
        );
      default:
...
export function assertValidRejectionReason(
  rejectionReason: string | undefined
): string {
  if (!rejectionReason?.trim()) {
    throw new BadRequestError(
      "Informe o motivo da rejeição em rejection_reason"
    );
  }
  ...
  if (trimmed.length > MAX_ADMIN_REQUEST_MESSAGE_LENGTH) {
    throw new BadRequestError(
      `O motivo da rejeição deve ter no máximo ${MAX_ADMIN_REQUEST_MESSAGE_LENGTH} caracteres`
    );
  }
