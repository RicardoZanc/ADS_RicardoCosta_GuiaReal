...
import type { AdminRequestItem, AdminRequestStatus } from "@/lib/types/adminRequests";
...
export default function AdminRequestsPage() {
  ...
  async function handleApprove(requestId: string) {
    ...
    try {
      await updateAdminRequest(requestId, { status: "APPROVED" });
      setRejectingId(null);
      setRejectionReason("");
      await loadRequests();
    } catch (err) {
          ...
          {requests.map((request) => (
            ...
            >
              ...
              <p className="mt-2 text-small text-muted">
                Candidato: <UserLink username={request.user.username} /> ·
                Reputação: {request.user.reputation_score}
              </p>
