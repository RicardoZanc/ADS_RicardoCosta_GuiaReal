...
import { assertCanReport, assertNoExistingReport, assertValidReportReason, loadReportTarget, } from "./reports.domainRules";
function toIsoString(date) {
    return (date ?? new Date(0)).toISOString();
}
async function findLinkedFactIds(targetType, targetId) {
    const evidence = await prisma.fact_evidence.findMany({
        where: targetType === "opinion"
            ? { opinion_id: targetId }
...
async function applyTargetModeration(targetType, targetId, status) {
...
const create = async (reporterId, input) => {
...
const list = async (query) => {
    ...
    return {
        data: reports.map((report) => {
            const targetType = report.target_opinion_id
                ? "opinion"
                : "thread";
            const target = targetType === "opinion" ? report.opinions : report.discussion_threads;
            return {
                id: report.id,
                reason: report.reason,
                status: report.status,
                ...
                target: {
                    type: targetType,
                    id: report.target_opinion_id ?? report.target_interaction_id ?? "",
                    title: targetType === "opinion"
...
const update = async (reportId, reviewerId, input) => {
    ...
    const targetType = report.target_opinion_id
        ? "opinion"
        : "thread";
    const targetId = report.target_opinion_id ?? report.target_interaction_id ?? "";
    if (!targetId) {
