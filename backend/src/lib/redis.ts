...
export async function connectRefreshTokenRedis(): Promise<void> {
  await refreshTokenRedis.connect();
}

export async function disconnectRefreshTokenRedis(): Promise<void> {
  if (refreshTokenRedis.isOpen) {
    await refreshTokenRedis.quit();
  }
}

export function registerRefreshTokenRedisShutdown(): void {
  const onShutdown = () => {
