...
export function authenticateToolApiKey(req, _res, next) {
    try {
        console.log('Headers: ', req.headers);
        const providedKey = req.headers[TOOL_API_KEY_HEADER];
        const key = typeof providedKey === "string" ? providedKey.trim() : undefined;
        if (!key) {
            logger.warn("Autenticação de ferramenta falhou: header X-Tool-Api-Key ausente ou inválido");
